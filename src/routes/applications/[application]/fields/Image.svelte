...
<div class="flex-1 flex flex-col gap-4">
  <div class="py-12 flex flex-col justify-center h-full">
    ...
    <form enctype="multipart/form-data" method="POST" action="?/file" bind:this={form} use:enhance={() => {
      ...
      <input
        disabled={disabled}
        name="file" accept="image/*" on:change={() => {
        form.requestSubmit()
    ...
    {#if value.value && value.value.length > 0}
      {#each value.value as { file, url }, idx}
        <div class="flex mt-1 gap-2">
          <div class="h-36 w-36 p-2 border rounded-md flex flex-col items-center justify-center">
            <a href={url} target="_blank" class="h-full w-full">
              <img alt={file} src={url} class="h-full w-full object-cover rounded-sm" />
            </a>
          </div>
          <Button
            disabled={disabled}
            size="icon" variant="ghost" on:click={() => {
