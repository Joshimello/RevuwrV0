...
<div class="flex-1 flex flex-col gap-4">
  <Card.Root class="py-12">
    <Card.Content class="pb-0 px-16 flex flex-col justify-center h-full">
      ...
      <Textarea class="mt-8 text-xl" placeholder="Type your answer here..." disabled />
    </Card.Content>
  </Card.Root>
  ...
  <div class="flex flex-col gap-4">
    ...
    <div class="flex items-center gap-2">
      <Checkbox bind:checked={value.isMaxChar} />
      <Label>Max characters</Label>
      {#if value.isMaxChar}
        <Input placeholder="0-99999" type="number" class="max-w-32 ml-4 h-4 p-1" bind:value={value.maxChar} />
      {/if}
    </div>
  </div>
</div>